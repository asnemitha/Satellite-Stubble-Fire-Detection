...
def test_condition_factor_derates_capacity():
    ...
    assert silted.full_flow_capacity() == pytest.approx(
        full.full_flow_capacity() * 0.5
    )
...
def test_rainfall_to_runoff_scales_with_imperviousness():
    ...
    runoff_low = rainfall_to_runoff(
        rain,
        low_imperv,
        cfg,
    )

    runoff_high = rainfall_to_runoff(
        rain,
        high_imperv,
        cfg,
    )
